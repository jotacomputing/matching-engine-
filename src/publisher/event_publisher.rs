...
impl EventPublisher {
    pub fn new( event_queue : Arc<SpscQueue<Event>> , mypubsub : RedisPubSubManager , pub_writter_order_event_queue : Arc<SpscQueue<OrderEvents>> ,  event_queue_from_engine_try : Consumer<Event>,event_queue_sender_to_writter_try : Producer<OrderEvents>) -> Self {
        Self { 
            event_queue  , 
            mypubsub , 
            pub_writter_order_event_queue,
...
//
            ...
            match self.event_queue_from_engine_try.try_pop(){
                ...
                Some(rec_event) => {
                    //println!("got the event from engine");
                    {
                        let ticker_message = TickerData::new(
                            String::from("ticker"), 
                            rec_event.market_update.symbol, 
