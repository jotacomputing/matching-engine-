// order evnts 
// rejected(insfuuicient funds , invalid symbol  , filled (completely ) , partial filled , cancelded)
...
impl OrderEventQueue {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self, QueueError> {
        ...
        // Cache both pointers
        let header_ptr = { mmap.as_mut_ptr() as *mut QueueHeader };
        let orders_ptr = unsafe { mmap.as_mut_ptr().add(HEADER_SIZE) as *mut OrderEvents };
    ...
    #[inline]
    pub fn dequeue(&mut self) -> Result<Option<OrderEvents>, QueueError> {
    ...
    pub fn enqueue(&mut self, order: OrderEvents) -> Result<(), QueueError> {
        let header = self.header_mut();
    ...
    pub fn dequeue_spin(&mut self, max_spins: usize) -> Result<Option<OrderEvents>, QueueError> {
